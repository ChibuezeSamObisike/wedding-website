...
const validateSongInput = (req, res, next) => {
...
router.post('/', validateSongInput, async (req, res) => {
    try {
...
router.get('/', async (req, res) => {
    try {
        const { status = 'approved', page = '1', limit = '10', artist, sort = 'createdAt', order = 'desc', } = req.query;
        const filter = {};
...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        const song = await Song.findByIdAndUpdate(id, { status }, { new: true, runValidators: true }).select('-__v');
        if (!song) {
            res.status(404).json({
                error: 'Song not found',
                message: 'No song found with the provided ID',
